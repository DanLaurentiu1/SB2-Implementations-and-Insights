...
# GIVEN
@pytest.fixture
def context(first_rng_16: np.random.Generator) -> ExplorationExploitationContext:
...
def test_pick_action(
    greedy_strategy: EpsilonGreedy, context: ExplorationExploitationContext
):
    # WHEN
    action_greedy = greedy_strategy.pick_action(context)
    ...
    # WHEN
    context.q_values = np.array([-2.0, 1.0, 0.0])
    action_greedy = greedy_strategy.pick_action(context)
